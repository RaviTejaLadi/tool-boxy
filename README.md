...
[![Live Demo](https://img.shields.io/badge/🌐_Live_Demo-tool--boxy.com-0ea5e9?style=for-the-badge)](https://tool-boxy.vercel.app/)
[![GitHub](https://img.shields.io/badge/GitHub-tool--boxy-181717?style=for-the-badge&logo=github)](https://github.com/tool-boxy/tool-boxy)
...
> **39+** focused utilities — pick a category and dive in.
...
```bash
# 1. Clone the repo
git clone https://github.com/tool-boxy/tool-boxy.git
cd tool-boxy
...
```text
src/
├── 🧩 components/     # Shared UI (shadcn), theme, syntax highlighting
├── ⚙️  config/        # Tool registry — titles, routes, categories
├── 🧰 features/       # One folder per tool (UI · stores · helpers)
├── 🪝 hooks/          # Shared hooks (e.g. favorites)
├── 🖼️  layouts/       # App shell & sidebar
...
Want to ship a new utility? Here’s the usual flow:

1. **Create** a feature folder under `src/features/YourTool/`
2. **Add** a page in `src/pages/YourToolPage.tsx`
3. **Register** the route in `src/App.tsx`
4. **List** it in `src/config/tools.ts` (title, description, category, icon)
5. **Verify** with `npm run pre-commit`
...
1. 🍴 Fork the repo
2. 🌿 Create your branch (`git checkout -b feat/amazing-tool`)
...
Copyright © 2026 Ravi Teja

---

<div align="center">

**Built for people who want the right utility without the friction.** 🧰✨
