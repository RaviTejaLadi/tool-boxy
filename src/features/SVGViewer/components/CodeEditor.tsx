...
export function CodeEditor() {
  ...
  const lineCount = Math.max(svgCode.split('\n').length, 1);
  ...
  useEffect(syncScroll, [svgCode, wordWrap]);
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLTextAreaElement>) => {
    ...
    if (e.key === 'Tab') {
      ...
      const next = `${svgCode.slice(0, selectionStart)}  ${svgCode.slice(selectionEnd)}`;
      setSvgCode(next, { history: false });
      scheduleCommit();
  ...
  return (
    <div className="relative min-h-0 flex-1 overflow-hidden bg-background">
      ...
      <textarea
        ...
        value={svgCode}
        onChange={(e) => {
          setSvgCode(e.target.value, { history: false });
          scheduleCommit();
        }}
