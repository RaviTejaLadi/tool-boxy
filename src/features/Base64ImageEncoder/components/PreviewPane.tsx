...
export function PreviewPane() {
  ...
  const isDragging = useEncoderStore((s) => s.isDragging);
  const addImages = useEncoderStore((s) => s.addImages);
  const selectImage = useEncoderStore((s) => s.selectImage);
  const setDragging = useEncoderStore((s) => s.setDragging);
            ...
            <div className="flex flex-wrap justify-center gap-2">
              {images.map((img) => {
                ...
                return (
                  ...
                  >
                    <img src={img.dataUri} alt={img.name} className="size-12 object-contain" />
                  </button>
                );
              })}
              <button
